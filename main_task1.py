...
from dataset_seq_task1 import AntiSpoofingDataset
from model_task1 import CNN_RNN
...
def val(data_loader, model, criterion, save_result=False):
    ...
    for (step, value) in enumerate(data_loader):
        ...
        # forward
        output = model(image).squeeze()
        all_scores = np.concatenate((all_scores, output.data.cpu().numpy()), axis=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoint/cnn-rnn_task1', help='directory to save logfile, checkpoint and output csv')
