...
from dataset_seq_task2 import AntiSpoofingDataset
from model_task2 import CNN_RNN
...
def main():
    ...
    if args.phase == 'train':
        ...
        for epoch in range(start_ep, args.epochs):
            ...
            if (epoch + 1) % 10 == 0:
                with torch.no_grad():
                    acc_val, auc_val, loss_val = val(val_loader, model, criterion)
        ...
        with torch.no_grad():
            acc_val, auc_val, loss_val = val(val_loader, model, criterion, save_result=True)
...
def val(data_loader, model, criterion, save_result=False):
    ...
    for (step, value) in enumerate(data_loader):
        ...
        score = torch.softmax(output, dim=1)[:, 0].data.cpu().numpy()
        all_scores = np.concatenate((all_scores, score), axis=0)
    ...
    AUC = -1
    if len(all_labels) > 0:
        all_labels = 1 - (all_labels > 0).astype(int)
        AUC = roc_auc_score(all_labels, all_scores)

    # logging
    if AUC != -1:
        print('[Val] Loss {loss.avg:.4f}\tAcc {acc.avg:.3f}\tAuc {auc:.3f}'.format(loss=losses, acc=ACC, auc=AUC))
    if args.phase == 'train':
        logger.write('[Val] Loss {loss.avg:.4f}\tAcc {acc.avg:.3f}\tAuc {auc:.3f}\n'.format(loss=losses, acc=ACC, auc=AUC))
    ...
    if save_result:
        with open(args.out_csv, 'w') as csv:
            ...
            for i in range(len(fnames)):
                #csv.write('{},{}\n'.format(fnames[i], all_scores[i]))
                csv.write('{},{}\n'.format(fnames[i], int(all_preds[i])))

    return ACC.avg, AUC, losses.avg
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoint/cnn-rnn_task2', help='directory to save logfile, checkpoint and output csv')
